...
def generate_pattern(phone, data):
    ...
    return {"data": pattern, "class": r.get("class:"+phone)}
...
def home(request):
    ...
    for phone in r.keys("+*"):
        ...
        data = raw_data[1:7*24+1]
        sigma = raw_data[7*24+1:7*24*2+1]
        pattern = generate_pattern(phone, data)
...
def analytics(request):
    ...
    hist = {}
    for phone in r.keys("deviation:+*"):
        key = phone.replace("deviation:+", "")
        pattern = raw_patterns["+" + key]
        hist[key] = {}
        hist[key]["data"] = list_to_js(list(enumerate(reversed(r.lrange(phone, 0, PERIOD-1)))))
        ...
        hist[key]["deviation_limits"] = list_to_js_i(list(enumerate(
            reversed([x.split(",") for x in r.lrange("deviation_limits:+"+key, 0, PERIOD-1)])
            )))
        hist[key]["diff"] = list_to_js(list(enumerate(reversed(r.lrange("diff:+"+key, 0, PERIOD-1)))))
    return render_to_response('analytics.html', {"deviation": hist})


def trends(request):
    raw_patterns = read_raw_patterns()
    hist = {}
    for cluster in r.keys("clusterTrend:*"):
