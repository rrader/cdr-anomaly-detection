...
def home(request):
    ...
    for phone in r.keys("+*"):
        ...
        raw_patterns[phone] = {"data": data, "sigma": 1.96*float(sigma)}
    ...
    for phone in r.keys("frequency*"):
        ...
        w = (last_hour//24 + 3)%7
        first = w*24+h
        p_value = lambda i: float(pattern["data"][(first + i) % (24*7)])
        hist[key]["pattern"] = list_to_js(list(enumerate([p_value(i) for i in range(0, PERIOD)])))
        hist[key]["appropriate"] = list_to_js_i(list(enumerate([(p_value(i) - pattern["sigma"], p_value(i) + pattern["sigma"]) for i in range(0, PERIOD)])))
